...
# 規約の版番号。文言を実質的に変更した場合はインクリメントし、
# 既に同意済みのユーザーにも再同意を求める（settings_store.accepted_terms_version と比較）。
...
【プライバシーに関する説明】
- 通常のローカル生成では、入力画像・生成動画は外部サーバへ送信されません
- ただし、利用者が任意で有効化する機能（Discord Bot、インターネット投入口/ブラウザ連携、
  アップデート確認、エラー報告）を使用する場合は、その機能に応じて外部と通信します
- 初回セットアップ時のほか、アップデート確認時にも通信が発生する場合があります
