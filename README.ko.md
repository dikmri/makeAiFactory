...
- **Discord Bot 연계** — Discord에 이미지를 올리면 동영상이 돌아오는 Bot을 앱 내에서 설정, 시작
- **완전 로컬 처리** — 인터넷 연결은 처음 설정만. 생성 데이터는 외부로 전송되지 않습니다.
- **자동 설정** — Python 환경, ComfyUI, 모델을 자동으로 구축합니다.
...
- **완성시의 자동 보존·통지음** — 완성된 동영상을 지정 폴더에 자동 보존, 완성시에 통지음을 재생(음량 조정 가능)
...
2. 이미지를 앱 창으로 드래그 앤 드롭(또는 `Ctrl+V`로 붙여넣기)
...
여러 채널을 지정하려면 동일한 단계를 반복하여 ID를 기록해 둡니다.
...
- 폴더 일괄 생성 중에는 Discord의 요청이 자동으로 거절됩니다 ( "폴더 생성 중이므로 허용되지 않습니다"라고 회신됩니다)
...
- 생성 콘텐츠의 이용·공개에 관한 책임은 모두 유저에게 귀속합니다
- 실재하는 인물의 동의 없는 성적 콘텐츠나 미성년을 대상으로 하는 콘텐츠의 생성을 금지합니다.
